...
public abstract class AudioManager<T> : SingletonMonoBehaviour<T> where T : MonoBehaviourWithInit {
	...
	/// 再生開始
	/// </summary>
	protected int RunPlayer(AudioClip audioClip, float volumeRate, float delay, float pitch, bool isLoop, Action callback = null)
	{
		AudioPlayer audioPlayer = GetNextAudioPlayer();
		audioPlayer.Play(audioClip, _baseVolume, volumeRate, delay, pitch, isLoop, NextID, callback);
	...
	/// 再生開始
	/// </summary>
	protected int RunPlayer(string audioPath, float volumeRate, float delay, float pitch, bool isLoop, Action callback = null) {
	...
	private AudioPlayer GetNextAudioPlayer() {
		...
		var nextAudioPlayer=_audioPlayerList
			.FirstOrDefault(audioPlayer => audioPlayer.CurrentState == AudioPlayer.State.Wait);
		if (nextAudioPlayer==null)
		{
			Debug.LogWarning($"Not Found Awaiting AudioPlayer.");
			int minID = _audioPlayerList.Min(audioPlayer => audioPlayer.ID);
			nextAudioPlayer = _audioPlayerList.FirstOrDefault(audioPlayer => audioPlayer.ID == minID);
		}
	...
	/// <summary>
	/// 指定されたIDのものが再生されていたら停止
	/// </summary>
	public void Stop(int audioPlayerID)
	{
		_audioPlayerList.FirstOrDefault(player => player.ID == audioPlayerID)?.Stop();
	}
