...
namespace InGame.Enemy.Logic
{
    ...
    public abstract class BaseEnemyLogic
    {
        ...
        private async void OnEatSweets(ISweets sweets)
        {
            ...
            int seID = SEManager.Instance.Play(SEPath.EAT_SWEETS,pitch:1.2f,isLoop:true);
            await sweets.BreakSweets(enemyEntity.eatUpTime,token);
            SEManager.Instance.Stop(seID);
            ResetEatState();
        }

        protected void ResetEatState()
        {
        ...
        protected virtual void HadPunched(Vector2 playerPos)
        {
            ...
            float toPlayerDirection = (enemyView.GetPosition().x - playerPos.x) >= 0 ? 1 : -1;
